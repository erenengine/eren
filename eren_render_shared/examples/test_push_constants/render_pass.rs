...
impl TestRenderPass {
    pub fn new(device: &Device) -> Self {
        ...
        let ubo_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: std::mem::size_of::<CameraUBO>() as wgpu::BufferAddress,
        ...
        let push_constant_range = wgpu::PushConstantRange {
            ...
            range: 0..std::mem::size_of::<PushModel>() as u32,
        };
    ...
    fn update_push_constants(&mut self, render_pass: &mut wgpu::RenderPass) {
        let time = self
            .start_time
        ...
        let push_model = PushModel { model };

        let model_bytes = unsafe {
            std::slice::from_raw_parts(
                &push_model as *const PushModel as *const u8,
                std::mem::size_of::<PushModel>(),
            )
        };

        render_pass.set_push_constants(wgpu::ShaderStages::VERTEX, 0, model_bytes);
    }
    ...
    pub fn record_commands(
        ...
        self.update_push_constants(&mut render_pass);
