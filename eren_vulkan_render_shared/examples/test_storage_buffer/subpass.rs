...
use super::{ssbo::StorageBufferObject, vertex::Vertex};
...
pub struct TestSubpass {
    ...
    storage_buffers_mapped: Vec<*mut std::ffi::c_void>,
...
impl TestSubpass {
    pub fn new(
        ...
        let mut storage_buffers_mapped = Vec::new();
        ...
        storage_buffers_mapped.resize(MAX_FRAMES_IN_FLIGHT, std::ptr::null_mut());
        ...
        for i in 0..MAX_FRAMES_IN_FLIGHT {
            ...
            storage_buffers_mapped[i] =
                device.map_memory(storage_buffers_memory[i], buffer_size)?;
    ...
    fn update_storage_buffer(
        ...
        // 메모리에 데이터 복사
        unsafe {
            let data_ptr = self.storage_buffers_mapped[frame_idx];
            std::ptr::copy_nonoverlapping(&ssbo, data_ptr as *mut StorageBufferObject, 1);
        }
    }
